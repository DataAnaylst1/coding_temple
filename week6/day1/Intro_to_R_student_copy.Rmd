...
a <-  "Hey, how are you?"
b <- "I'm fine, and you?"
sub("fine", "fantastic", b)

sprintf('There are %d students in the %s', 10,'classroom')
```
...
```{r}
# Create a seq from 5 to 15, increase it by increments of .3:
...
```{r}
vect_3 <- seq(2,96, by=20)
vect_4 <- seq(-6,32, by=8)
vect_3
vect_4
vect_3* vect_4
vect_3 ** vect_4
...
```{r}
str(df) # This is going to go through the dataframe and give the values with their datatypes
```
...
my_matrix <- matrix(
  ...
  nrow=2,
  ncol=5,
  dimnames=list(c("Top Half Numbers", "Lower Half Numbers"))
)
my_matrix
```
...
my_matrix <- matrix(
  ...
  ncol=5,
  byrow=TRUE,
  dimnames=list(c("Top Half Numbers", "Lower Half Numbers"), c('Col 1', 'Col 2', 'Col 3',"Col 4","Col 5")))
my_matrix
```
...
```{r}
my_array <- array(
  c(124,254,116,123,932,102,65,85,91),
  # Rows by columns by matricies
  dim=c(3,3,3),
...
```{r}
# Grab rows 2 and 3 of each matrix:
my_array[2:3,,]
```
